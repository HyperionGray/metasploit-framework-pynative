...
unless ARGV.include?('-q') || ENV['MSF_QUIET'] == '1'
  $stderr.puts ""
  $stderr.puts "🐍 Metasploit Framework Daemon - Traditional Ruby Version"
  $stderr.puts ""
  $stderr.puts "💡 For an enhanced Python experience, try:"
  $stderr.puts "   source msfrc          # Activate MSF shell environment"
  $stderr.puts "   msf_rpc               # Python-enhanced RPC server"
  $stderr.puts ""
  $stderr.puts "   Or use -q flag to hide this message"
  $stderr.puts ""
end
