...
# Script to split Windows API constants into categorized files
require 'fileutils'
...
def categorize_constant(const_name)
  ...
  when /^(FILE_|GENERIC_|CREATE_|OPEN_|SHARE_|SECURITY_|ACCESS_|STANDARD_RIGHTS_)/
    'file_security'
  when /^(PROCESS_|THREAD_|TOKEN_|SE_|PRIVILEGE_|SECURITY_)/
    'process_security'
  when /^(DNS_|NS_|WSAE|AF_|PF_|SOCK_|IPPROTO_|IF_TYPE_|IN_CLASS)/
    'network_dns'
...
def split_constants_file(input_file, output_dir)
  ...
  loader_file = File.join(File.dirname(input_file), 'api_constants_new.rb')
  ...
  puts "\nCreated main loader file: #{loader_file}"
