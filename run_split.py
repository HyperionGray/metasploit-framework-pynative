...
for const in constants:
    ...
    if re.match(r'^ERROR_', name):
        categories['errors'].append(const)
    elif re.match(r'^(WM_|HWND_|WS_|SW_|SWP_|VK_|IDC_|IDI_|MB_|IDOK|IDCANCEL|IDYES|IDNO)', name):
        categories['ui_windows'].append(const)
    elif re.match(r'^(REG|HKEY_|KEY_)', name):
        categories['registry'].append(const)
    elif re.match(r'^(FILE_|GENERIC_|CREATE_|OPEN_|SHARE_)', name):
        categories['file_io'].append(const)
    elif re.match(r'^(PROCESS_|THREAD_|TOKEN_|SE_)', name):
        categories['process_security'].append(const)
    elif re.match(r'^(SERVICE_|SC_)', name):
        categories['services'].append(const)
    elif re.match(r'^(DNS_|SOCKET_|AF_|SOCK_|IPPROTO_|TCP_|UDP_|IP_|WSAE)', name):
        categories['network'].append(const)
    elif re.match(r'^(CERT_|CRYPT|ALG_|CALG_|PROV_)', name):
        categories['cryptography'].append(const)
    elif re.match(r'^(LANG_|SUBLANG_|SORT_|LOCALE_)', name):
        categories['locale'].append(const)
    elif re.match(r'^(SQL_|ODBC_)', name):
        categories['database'].append(const)
    elif re.match(r'^(PRINTER_|PRINT_|DM_)', name):
        categories['printing'].append(const)
    elif re.match(r'^(MCI_|WAVE_|MIDI_)', name):
        categories['multimedia'].append(const)
    elif re.match(r'^(IMAGE_|PE_|SECTION_)', name):
        categories['pe_format'].append(const)
    elif re.match(r'^(TAPE_|IOCTL_)', name):
        categories['device_io'].append(const)
    elif re.match(r'^(RPC_|NDR_)', name):
        categories['rpc'].append(const)
    elif re.match(r'^SNMP_', name):
        categories['snmp'].append(const)
    elif re.match(r'^(HTTP_|INTERNET_|WINHTTP_)', name):
        categories['http_internet'].append(const)
    elif re.match(r'^(SECURITY_|DOMAIN_|POLICY_)', name):
        categories['security_policy'].append(const)
    else:
        categories['misc'].append(const)
